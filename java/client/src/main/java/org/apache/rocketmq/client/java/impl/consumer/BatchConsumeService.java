 ...
 * <ol>
 *   <li>The buffered message count reaches {@link BatchPolicy#getMaxBatchSize()}.</li>
...
public class BatchConsumeService extends ConsumeService {
    ...
    public void consume(ProcessQueue pq, List<MessageViewImpl> messageViews) {
        ...
        try {
            ...
            while (buffer.isBatchReady(batchPolicy.getMaxBatchSize(), batchPolicy.getMaxBatchBytes())) {
                flushBatch();
            }
        } finally {
            buffer.unlock();
    ...
    /**
     * Takes one batch from the buffer (up to maxBatchSize / maxBatchBytes) and submits it
     * for consumption.
     * <p>
    ...
    private void flushBatch() {
        final List<BatchBuffer.BufferedMessage> batch = buffer.takeBatch(
            batchPolicy.getMaxBatchSize(), batchPolicy.getMaxBatchBytes());
        if (batch.isEmpty()) {
            return;
        }
        submitBatch(batch);
