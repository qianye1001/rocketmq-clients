...
class BatchBuffer {
     ...
     */
    List<BufferedMessage> takeBatch(int maxBatchSize, long maxBatchBytes) {
        final List<BufferedMessage> batch = new ArrayList<>();
        long batchBytes = 0;

        while (!this.isEmpty() && batch.size() < maxBatchSize) {
     ...
     */
    boolean isBatchReady(int maxBatchSize, long maxBatchBytes) {
        if (messages.isEmpty()) {
            return false;
        }
        return messages.size() >= maxBatchSize || accumulatedBytes >= maxBatchBytes;
    }
